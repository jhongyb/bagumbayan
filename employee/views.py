...
def department_group_dtr(request,dpt):
            if request.method=='POST':
                depart=Department.objects.get(pk=dpt)
                employee=Employee.objects.filter(dept=depart.id)
                content=[]
                a=range(1,32)
                punches=Biometric.objects.filter(bio_date__range=(opt['startdate'],opt['enddate'])).values(
                    'bio_date','bio_punchstate','bio_time'
                )
                if punches:
                    df=pd.DataFrame(list(punches))
                ...
                return render(request,'employee/Dtr_Employees.html')
