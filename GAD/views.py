...
def rbim(request):
    ...
    try:
        ...
        stat = requests.get("http://localhost:1000/api/rbim", headers=headers)
        
        if stat.status_code == 200:
            data = stat.json()
            return render(request, 'gad.html', {'items': data})
        else:
            return HttpResponse(f"API Error: Received status code {stat.status_code}", status=stat.status_code)
            
    except requests.exceptions.RequestException as e:
        return HttpResponse(f"Failed to connect to API: {e}", status=500)